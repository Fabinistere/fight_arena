...
pub fn create_dialog_panel_on_key_press(
    ...
    if keyboard_input.just_pressed(KeyCode::O) {
        if let Ok((_entity, animator, _style)) = query.get_single() {
            ...
            next_game_state.set(GameState::DialogWall);
        }
    }
}
...
pub fn create_dialog_panel_on_combat_event(
    ...
    animated_dialog_panel_query: Query<(Entity, &Animator<Style>, &Style), With<DialogPanel>>,

    mut current_interlocutor: ResMut<CurrentInterlocutor>,
    mut next_game_state: ResMut<NextState<GameState>>,
) {
    for CombatEvent { entity } in ev_combat.iter() {
        // TODO: feature - if already open go to combat tab
        if animated_dialog_panel_query.is_empty() {
            current_interlocutor.interlocutor = Some(*entity);
            next_game_state.set(GameState::DialogWall);
        }
    }
}
